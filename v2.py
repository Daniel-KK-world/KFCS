...
class AttendanceSystem:
    ...
    def start_workers(self):
        """Start multiple face processing workers"""
        for _ in range(2):  # Use 2 worker processes
            worker = FaceProcessingWorker(self.frame_queue, self.result_queue)
            worker.start()
    ...
    # ... [Keep all your other existing AttendanceSystem methods unchanged] ...

class FaceProcessingWorker(mp.Process):
...
class AttendanceUI:
    ...
    def setup_ui(self):
        ...
        # [Keep all your existing UI setup code unchanged]
        # ...
    
    def process_webcam(self):
        """Optimized webcam processing pipeline"""
        ...
        # Send frame to workers if queue not full
        if not self.attendance_system.frame_queue.full():
            self.attendance_system.frame_queue.put(frame.copy())
        
        # Check for results
        try:
            result = self.attendance_system.result_queue.get_nowait()
            self.process_detection_result(frame, result)
        except queue.Empty:
            pass
        ...
        if self.frame_count % 10 == 0:
            fps = 10 / (time.time() - self.last_frame_time)
            self.last_frame_time = time.time()
    ...
    def process_detection_result(self, frame, result):
        ...
        for (loc, encoding) in zip(result['locations'], result['encodings']):
    ...
    def on_close(self):
        """Cleanup on exit"""
        self.attendance_system.stop_workers()
...
class AttendanceUI:
    ...
    def on_close(self):
        ...
        if hasattr(self, 'workers'):
            for worker in self.workers:
                worker.terminate()  # Force-stop the process
    ...
    def process_webcam(self):
        ...
        while True:
            ...
            # STEP 1: Send frame to workers (non-blocking)
            if not self.frame_queue.full():
                self.frame_queue.put(frame.copy())
            
            # STEP 2: Check for results (non-blocking)
            try:
                result = self.result_queue.get_nowait()
                self.draw_face_boxes(frame, result['locations'])
            except queue.Empty:
                pass
